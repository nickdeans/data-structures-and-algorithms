...
const templateWithJQuery = () => {
  starWarsPeople.forEach(person => {
    let create = $('#template').clone();
    $(create).find('h2').text(`${person.name}`);
    $(create).find('h3').text(`${person.height}`);
    $(create).find('p').text(`${person.eye_color}`);
    $(create).removeAttr('id');
    $('main').append(create);
  });
}
...
const howMuchPencil = (str) => {
  let result = [];
  let stringChange = str;
  result.push(str);
  for(let i=0; i<str.length; i++){
    stringChange = stringChange.slice(1);
    result.push(stringChange);
  }
  return result;
};
...
const wordsToCharList = (arr) => {
  ...
  for (let i=0; i<arr.length; i++){
    result.push(change.substring(0,1));
