...
const createServer = () => {
  ...
  app.get('/aboutme', function(req,res){
    res.send('I am Nick and I attend Code Fellows')
  });
  app.get('/favoritefoods', function(req,res){
    res.send('pizza, sushi, and chicken')
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  let arrayOne = []
  input.forEach(arr => {
    let arrayTwo = arr.filter(value => typeof(value) === 'number');
    let arrayThree = arrayTwo.filter(a => a % 5 === 0);
    let arrayFour = arrayThree.map(x => Math.pow(2,x));
    arrayOne.push(arrayFour);
  })
  return arrayOne;
};
